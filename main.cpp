...
void HandleBulletThreats(MainObject &p_player, vector<auto> &threats_list, ExplosionObject &exp_main, Map& map_data, PlayerPower &player_power) {
    for(int i = 0; i < threats_list.size(); i++) {
        ...
        if(p_threat != NULL) {
            ...
            p_threat->MakeBullet(g_screen, SCREEN_WIDTH, SCREEN_HEIGHT, map_data, p_player.get_space());
...
void HandlePlayer(MainObject& p_player, Map& map_data, PlayerPower& player_power) {
    ...
    p_player.set_space(map_data.start_x_ - p_player.get_map_x());
    ...
    if(p_player.get_win()) {
        if(MessageBox(NULL, "YOU WIN!", "Congratulation!", MB_OK | MB_ICONSTOP ) == IDOK) {
            p_player.Free();
            close();
        }
    }
